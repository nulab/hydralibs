        ...
        if (opts && opts.tag && opts.takeLatest) {
          ...
          ret
            .then((d) => {
              ...
              if (latestRecorded === timestamp) setState((state: S) => d(state))
            })
            .then(resolve)
            .catch(reject)
        } else
          ret
            .then((d) => setState((state: S) => d(state)))
            .then(resolve)
            .catch(reject)
        return state
  ...
  let dispatch = ((update: Update<S>, opts?: DispatchOpts) => {
    ...
    if (isTransitions(update)) {
      runUpdateFn(update[0], setState, tracker, dispatchOpts)
        .then(() => {
          if (update.length > 1) dispatch(update.slice(1), dispatchOpts)
        })
        .catch((err) =>
          console.error(
            `An error happened while running the transitions ${
              err.message || err
            }`
          )
        )
    } else {
      runUpdateFn(update, setState, tracker, dispatchOpts).catch((err) =>
        console.error(
          `An error happened while running the function ${err.message || err}`
        )
      )
    }
  }) as Dispatch<S>
